...
//        join 후 department를 이용하기가 불편.
        List<Employee> employeeList2 = queryFactory.selectFrom(qEmployee)
                .join(qEmployee.department)
                .where(qEmployee.department.name.ne("test"))
                .fetch();

//        qEmployee.department 대신 별칭 지정 할 수 있다.
        List<Employee> employeeList3 = queryFactory.selectFrom(qEmployee)
    ...
    void whereJoin(){
//        where의 조건에 연관관계ㅒ를 맺은 엔티티를 사용하면 자동 join을 사용한다.(pk=fk조건)
...
//        카르테시안 곱으로 조인하는 경우 where의 조건을 활용하여 원하는 데이터를 골라내야한다.
        ...
        // 예전 버전에서는 연관관계가 없는 조인이 불가능하여 어쩔 수 없이 사용하는 경우가 있었으나
